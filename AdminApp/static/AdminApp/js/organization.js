...
$(document).ready(function () {
    ...
    $('body').on('click', '.category-delete-link', function (e) {
        ...
        $.ajax({
            url: '/admin/get_organization/' + id,
            method: 'get',
            dataType: 'json',
            success:
                function (response) {
                    inner_form.html(response.html)
                    form.attr({'action': '/admin/about/delete/' + response.id + '/'})
